...
namespace XpTIlbudsapp.Persistency
{
    ...
    class PersistencyService2
    {
        ...
        public static async Task<ObservableCollection<Vare>> LoadNotesFromJsonAsync()
        {
            string notesJsonString = await DeserializeNotesFileAsync(JsonFileName);
            if (notesJsonString != null)
                return (ObservableCollection<Vare>) JsonConvert.DeserializeObject(notesJsonString, typeof (ObservableCollection<Vare>));
            return null;
        }
